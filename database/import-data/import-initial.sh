...
# Download of the PBF
download_pbf() {
    wget -P $WORKING_DIR_DATABASE/database/import $URL_PBF_DATABASE
}

# Download of the state.txt file
download_state() {
    wget -P $WORKING_DIR_DATABASE/database/osmosis $URL_PBF_STATE_DATABASE
    mv $WORKING_DIR_DATABASE/database/osmosis/$PBF_STATE_NAME_DATABASE $WORKING_DIR_DATABASE/database/osmosis/state.txt
...
create_config() {
    export CACHEDIR_DATABASE="$WORKING_DIR_DATABASE/database/cache"
    ...
    export MAPPING_DATABASE="$WORKING_DIR_DATABASE/database/config/mapping.yml"
    cat > $WORKING_DIR_DATABASE/database/config/config.json << EOF1
{
...
import_pbf() {
    imposm3 import -diff -config $WORKING_DIR_DATABASE/database/config/config.json -read $WORKING_DIR_DATABASE/database/import/$PBF_NAME_DATABASE -write
}

# Deployment of the import
deploy_production() {
    imposm3 import -config $WORKING_DIR_DATABASE/database/config/config.json -deployproduction
}
